 ...
 */
 ...
 class AppSpHelper{
    private static final String FILE_NAME="doctor_sp";
     ...
     * 获取SP实例
     *
     * @return {@link AppSpHelper}
     */
    protected static AppSpHelper getInstance() {
        return getInstance("");
    }
     ...
     * @return {@link AppSpHelper}
     */
    protected static AppSpHelper getInstance(String spName) {
        if (isSpace(spName)) spName = FILE_NAME;
        AppSpHelper sp = sSPMap.get(spName);
        if (sp == null) {
            sp = new AppSpHelper(spName);
            sSPMap.put(spName, sp);
        }
        return sp;
    }

    private AppSpHelper(String spName) {
     ...
     * @param values 值
     */
    protected void put(@NonNull String key, @NonNull Set<String> values) {
     ...
     * @return 存在返回对应值，不存在返回默认值{@code defaultValue}
     */
    protected Set<String> getStringSet(@NonNull String key, @NonNull Set<String> defaultValue) {
    ...
    private static boolean isSpace(String s) {
        ...
        for (int i = 0, len = s.length(); i < len; ++i) {
            if (!Character.isWhitespace(s.charAt(i))) {//判断是否为空格
                return false;
            }
        }
        return true;
