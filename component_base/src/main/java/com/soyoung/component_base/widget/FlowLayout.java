...
public class FlowLayout extends ViewGroup {
  ...
  private int lineNum;
  private int mVspace;
  private FlowLayoutLineNum mFlowLayoutLineNum;
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    if (orientation == HORIZONTAL) {
      if (singleLine) {
        this.setMeasuredDimension(resolveSize(controlMaxLength, widthMeasureSpec),
            resolveSize(singleLineHeight, heightMeasureSpec));
      } else if (twoLine) {
        if (lineNum >= 2) {
          this.setMeasuredDimension(resolveSize(controlMaxLength, widthMeasureSpec),
              resolveSize(singleLineHeight * 2 + singleLineHeight - mVspace,
                  heightMeasureSpec));
        } else {
          this.setMeasuredDimension(resolveSize(controlMaxLength, widthMeasureSpec),
              resolveSize(singleLineHeight, heightMeasureSpec));
    ...
    if (mFlowLayoutLineNum != null) {
      mFlowLayoutLineNum.getLineNum(lineNum);
    }
  }
  ...
  private void readStyleParameters(Context context, AttributeSet attributeSet) {
    ...
    try {
      ...
      orientation = a.getInteger(R.styleable.FlowLayout_flowLayout_orientation, HORIZONTAL);
      debugDraw = a.getBoolean(R.styleable.FlowLayout_debugDraw, false);
      singleLine = a.getBoolean(R.styleable.FlowLayout_singleLine, false);
      twoLine = a.getBoolean(R.styleable.FlowLayout_twoLine, false);
    } finally {
      a.recycle();
    }
  }
  ...
  public void setLineNum(FlowLayoutLineNum mFlowLayoutLineNum) {
    this.mFlowLayoutLineNum = mFlowLayoutLineNum;
  }

  public interface FlowLayoutLineNum {
