...
import com.soyoung.component_base.util.SystemUtils;
import com.soyoung.component_base.widget.tablayout.utils.UnreadMsgUtils;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mIndicatorText_bottom = ta.getBoolean(R.styleable.SlidingTabLayout_tl_indicator_text_bottom, false);
    ...
    private void addTab(final int position, String title, View tabView) {
        ...
        if (mDiscoverTabStyles) {
            tabView.setBackgroundResource(R.drawable.discover_video_tab_selector);
            int length = tv_tab_title.getText().length();
            if (length < 4 && length > 0) {
                tv_tab_title.setPadding(SystemUtils.pxToDp(mContext, 13), SystemUtils.pxToDp(mContext, 4),
                        SystemUtils.pxToDp(mContext, 13), SystemUtils.pxToDp(mContext, 4));
            } else if (length >= 4) {
                tv_tab_title.setPadding(5, SystemUtils.pxToDp(mContext, 4),
                        5, SystemUtils.pxToDp(mContext, 4));
            }
    ...
    private boolean mDiscoverTabStyles = false;

    public void userDiscoverTabStyles(boolean discoverTabStyles) {
        this.mDiscoverTabStyles = discoverTabStyles;
    }
        ...
        for (int i = 0; i < mTabCount; i++) {
            ...
            if (tv_tab_title != null) {
                ...
                if (mTextBold == TEXT_BOLD_BOTH) {
                    tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextsize + 2);
                    tv_tab_title.getPaint().setFakeBoldText(true);
                } else if (mTextBold == TEXT_BOLD_NONE) {
                    tv_tab_title.getPaint().setFakeBoldText(false);
                    tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextsize);
                }
                if(i == mCurrentTab && mTextSelectSize != 0){
    ...
    private void scrollToCurrentTab() {
        ...
        View childAt = mTabsContainer.getChildAt(mCurrentTab);
        int offset = (int) (mCurrentPositionOffset * childAt.getWidth());
    ...
    /*设置当前选中的tab*/
    public void setCurrentTabSelect(int position) {
        updateTabSelection(position);
    }
        ...
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tab_title != null) {
                ...
                if (mTextBold == TEXT_BOLD_WHEN_SELECT) {
                    ...
                    tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextsize + 2);
                }
                if(i == mCurrentTab && mTextSelectSize != 0){
                    tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextSelectSize);
//                    setIndicatorWidth(mIndicatorWidth*(mTextSelectSize/mTextsize));
//                    setIndicatorWidth(mIndicatorWidth*1.3f);
                }else{
                    tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextsize);
//                    setIndicatorWidth(mIndicatorWidth*0.75f);
                }
            }
        }
    }
    ...
    private void calcIndicatorRect() {
        ...
        if (mIndicatorStyle == STYLE_NORMAL && mIndicatorWidthEqualTitle) {
            ...
            mTextPaint.setTextSize(mTextSelectSize == 0?mTextsize:mTextSelectSize);
        ...
        mIndicatorRect.top = (int) (getHeight() - ((getHeight() - (mTextSelectSize == 0 ? mTextsize
            : mTextSelectSize)) / 2));
        /* for mIndicatorWidthEqualTitle */
        if (mIndicatorStyle == STYLE_NORMAL && mIndicatorWidthEqualTitle) {
        ...
        if (mIndicatorStyle == STYLE_TRIANGLE) {
            ...
            if (mIndicatorHeight > 0) {
                ...
                if (mIndicatorGravity == Gravity.BOTTOM) {
                    if (mIndicatorText_bottom){
                        mIndicatorDrawable.setBounds(paddingLeft + (int) mIndicatorMarginLeft + mIndicatorRect.left,
                                mIndicatorRect.top,
                                paddingLeft + mIndicatorRect.right - (int) mIndicatorMarginRight,
                                (int) (mIndicatorRect.top+mIndicatorHeight));
                    }else {
                        mIndicatorDrawable.setBounds(paddingLeft + (int) mIndicatorMarginLeft + mIndicatorRect.left,
     ...
     * @param txt      显示的文字
     */
    public void showMsg(int position, String txt) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        ...
        MsgView tipView = (MsgView) tabView.findViewById(R.id.rtv_msg_tip);
        if (tipView != null) {
            UnreadMsgUtils.show(tipView, txt);

            if (mInitSetMap.get(position) != null && mInitSetMap.get(position)) {
                return;
            }

            setMsgMargin(position, 6, 8);
            mInitSetMap.put(position, true);
        }
    }
     ...
     * @param customPadding
     */
    public void showDot(int position, boolean customPadding) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        View tabView = mTabsContainer.getChildAt(position);
        MsgView tipView = (MsgView) tabView.findViewById(R.id.rtv_msg_tip);
        if (tipView != null) {
            UnreadMsgUtils.show(tipView, 0);
            if (mInitSetMap.get(position) != null && mInitSetMap.get(position)) {
                return;
            }
            setMsgMargin(position, 2, 10);
            mInitSetMap.put(position, true);
        }
    }

    public void showDotTopRight(int position) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
     ...
     * @param position 显示tab位置
     */
    public void showHotView(int position) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }

        View tabView = mTabsContainer.getChildAt(position);
        ImageView rtv_hot_tip = tabView.findViewById(R.id.rtv_hot_tip);
        rtv_hot_tip.setVisibility(VISIBLE);
    }
    ...
    public void showDot(int position) {
        ...
        if (tipView != null) {
            ...
            lp.leftMargin = mTabWidth >= 0 ? (int) (mTabWidth / 2 + textWidth / 2 + dp2px(4)) : (int) (mTabPadding + textWidth + dp2px(4));
            lp.topMargin = mHeight > 0 ? (int) (mHeight - textHeight) / 2 - dp2px(2) :(int) (textHeight/2)-5 ;
    ...
    public int dotVisibility(int position) {
        int result = -1;
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
    ...
    public void setMsgMarginTopRight(int position) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        final View tabView = mTabsContainer.getChildAt(position);
        final MsgView tipView = (MsgView) tabView.findViewById(R.id.rtv_msg_tip);
        final TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
        if (tabView != null) {
            tabView.post(new Runnable() {
                @Override public void run() {
                    MarginLayoutParams lp = (MarginLayoutParams) tipView.getLayoutParams();
                    int width = tabView.getWidth() / 2 + tv_tab_title.getWidth() / 2;
                    lp.leftMargin = mTabPadding <= 0 ? width : width - ((int) (mTabPadding));
                    int height = tabView.getHeight() - (tabView.getHeight() / 2
                        + tv_tab_title.getHeight() / 2);
                    lp.topMargin = height > 5 ? height - 5 : height;
                    tipView.setLayoutParams(lp);
                }
            });
        }
    ...
    public void setMsgToParentTopRight(int position) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        final View tabView = mTabsContainer.getChildAt(position);
        ...
        if (tabView != null) {
            tabView.post(new Runnable() {
                @Override public void run() {
                    MarginLayoutParams lp = (MarginLayoutParams) tipView.getLayoutParams();
                    int width = tabView.getWidth() / 2 + tv_tab_title.getWidth() / 2;
                    width = width - SizeUtils.dp2px(tabView.getContext(), 4);
                    if (width + tipView.getWidth() > tabView.getWidth()) {
                        lp.leftMargin = tabView.getWidth() - tipView.getWidth();
                    } else {
                        lp.leftMargin = mTabPadding <= 0 ? width : width - ((int) (mTabPadding));
                    }
                    lp.topMargin = 0;
